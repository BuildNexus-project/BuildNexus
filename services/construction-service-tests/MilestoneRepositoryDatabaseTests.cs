...
public class MilestoneRepositoryDatabaseTests
{
    ...
    [Fact]
    public async Task Timestamps_come_back_from_the_reader_stamped_as_UTC()
    {
        // The bug this pins: MySQL DATETIME has no timezone attached, so
        // MySqlConnector reads it back as DateTimeKind.Unspecified. System
        // .Text.Json then serializes Unspecified without a Z suffix, and
        // the browser reads that suffix-less ISO string as local time — so
        // "Last updated" would display 5h 30m off in Colombo after a
        // reload. The fix stamps Kind = Utc when Map() reads the column;
        // this test proves the round trip (write UTC -> store as
        // DATETIME(6) -> read back) preserves Kind, not just the wall
        // -clock value.
        var projectId = _fixture.ProjectId("b16");
        await _fixture.PlantApprovedDesignAsync(projectId);

        await _fixture.MilestoneRepository.CreateAsync(projectId, "Foundation poured");

        // A fresh CreateAsync return is already UTC in memory (never touched
        // the reader). This re-read is what forces the value through the
        // reader path — the code path where the bug lived.
        var reread = Assert.Single(await _fixture.MilestoneRepository.ListForProjectAsync(projectId));

        Assert.Equal(DateTimeKind.Utc, reread.CreatedAtUtc.Kind);
        Assert.Equal(DateTimeKind.Utc, reread.UpdatedAtUtc.Kind);
    }
    ...
    [Fact]
    public async Task Template_creates_all_seven_canonical_milestones_on_a_fresh_project()
    {
        // Definition of Done: a PM can plant the canonical build in one
        // click. This is the happy path — no existing milestones, all seven
        // land in order.
        var projectId = _fixture.ProjectId("b17");
        await _fixture.PlantApprovedDesignAsync(projectId);

        var result = await _fixture.MilestoneRepository.CreateFromTemplateAsync(
            projectId, MilestoneTemplates.Canonical);

        Assert.NotNull(result);
        Assert.Equal(MilestoneTemplates.Canonical.Count, result!.Count);
        Assert.Equal(MilestoneTemplates.Canonical, result.Select(m => m.Name).ToList());
        Assert.All(result, m => Assert.Equal(MilestoneStatus.NotStarted, m.Status));
    }

    [Fact]
    public async Task Template_is_idempotent_and_only_inserts_missing_names()
    {
        // The reason the endpoint exists at all — a PM should be able to
        // click twice, or apply the template after having typed one or two
        // by hand, without a duplicate-name conflict. Names already on the
        // project are silently skipped; the new ones land alongside them.
        var projectId = _fixture.ProjectId("b18");
        await _fixture.PlantApprovedDesignAsync(projectId);

        // Plant two of the template names by hand first.
        await _fixture.MilestoneRepository.CreateAsync(projectId, "Foundation");
        await _fixture.MilestoneRepository.CreateAsync(projectId, "Walls");

        var result = await _fixture.MilestoneRepository.CreateFromTemplateAsync(
            projectId, MilestoneTemplates.Canonical);

        Assert.NotNull(result);
        // All seven template names are present after the call — the two
        // that were already there and the five newly-inserted ones.
        Assert.Equal(
            MilestoneTemplates.Canonical.OrderBy(n => n),
            result!.Select(m => m.Name).OrderBy(n => n));

        // And the project's total (which would include any non-template
        // additions) is exactly seven — no duplicates.
        var full = await _fixture.MilestoneRepository.ListForProjectAsync(projectId);
        Assert.Equal(7, full.Count);
    }

    [Fact]
    public async Task Template_returns_null_when_the_design_has_not_been_approved()
    {
        // Same AC-1 gate as CreateAsync — the template path cannot bypass
        // the design-approval check. No milestone_setups row → null →
        // controller returns 400.
        var projectId = _fixture.ProjectId("b19");
        // Deliberately no PlantApprovedDesignAsync call.
        ...
        Assert.Null(result);
        // And no rows were written — the transaction rolled back before
        // any INSERT touched the table.
        Assert.Empty(await _fixture.MilestoneRepository.ListForProjectAsync(projectId));
    }
}