...
public class EndpointRoleDeclarationTests
{
    ...
    [Fact]
    public void Uploading_a_design_document_is_the_architects_alone()
    {
        // US-09 is "As an Architect, I want to upload...". A Client watches the
        // work; staff on the delivery side and an Admin do not submit it on the
        // Architect's behalf. Pinned rather than left to whoever edits the
        // controller next.
        Assert.Equal([PlatformRoles.Architect], RolesFor("Upload"));
    }

    [Fact]
    public void Reading_design_documents_is_open_to_every_role()
    {
        // The owning Client reviews the latest work, and the staff on the
        // project need it too — so the role is not what decides the answer, the
        // per-project check against the Project Service is. The route only has
        // to let the four roles reach it.
        Assert.Equal(PlatformRoles.All, RolesFor("ListForProject"));
