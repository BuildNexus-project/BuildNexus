...
/// <summary>
/// ADO.NET data access for the <c>project_outbox_events</c> table. Direct SQL
...
public class OutboxRepository : IOutboxRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // committed in — not by occurred_at, which holds whole seconds and ties
        // for two events raised by the same status change.
        const string sql = $@"
            SELECT {SelectColumns}
            FROM project_outbox_events
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // picks it up again. The message is truncated to what the column holds:
        // a broker error can arrive with a long chain of inner detail, and
        // failing to record the attempt because the reason was too wordy would
        // lose the very thing being recorded.
        const string sql = @"
            UPDATE project_outbox_events
    ...
    public async Task<IReadOnlyList<OutboxEvent>> ListForProjectAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        // Oldest first, so it reads as the order things were announced in — the
        // same way the status history reads.
        const string sql = $@"
            SELECT {SelectColumns}
            FROM project_outbox_events
    ...
    /// <c>last_error</c> are absent on purpose: the database numbers the row,
    /// and a freshly enqueued event has been tried zero times and has not yet
    /// failed at anything.
    /// </para>
    /// </remarks>
    ...
    {
        const string sql = @"
            INSERT INTO project_outbox_events
