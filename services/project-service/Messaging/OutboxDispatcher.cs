...
public sealed class OutboxDispatcher : BackgroundService
{
    ...
    private readonly IProjectEventPublisher _publisher;
    private readonly ILogger<OutboxDispatcher> _logger;
    private readonly OutboxOptions _options;

    public OutboxDispatcher(
        IServiceScopeFactory scopeFactory,
        IProjectEventPublisher publisher,
    ...
    /// </summary>
    /// <remarks>
    /// Stops at the first failure rather than skipping past it. Events are
    /// ordered, and a project's <c>ProjectApproved</c> arriving while the
    /// <c>ProjectUpdated</c> that carried the same move is still stuck would
    /// show a consumer a state the project never passed through.
    /// <para>
    /// That does mean one undeliverable event holds up everything behind it.
    /// The trade is deliberate: in practice a failure here is the broker being
    /// unreachable, in which case the events behind it would fail too, and the
    /// alternative — publishing out of order to keep the queue moving — trades
    /// a delay every consumer recovers from for corruption none of them can
    /// detect. A genuinely poisoned event is visible as a rising
    /// <c>attempt_count</c> with the reason beside it, which is what the events
