...
/// <summary>
/// ADO.NET data access for the <c>payment_outbox_events</c> table. Direct SQL
/// only — no ORM. Every value reaches MySQL as a bound parameter.
/// </summary>
public class OutboxRepository : IOutboxRepository
{
    private const string SelectColumns =
        "sequence_number, id, project_id, invoice_id, event_type, envelope, occurred_at, published_at, "
    ...
    public async Task<IReadOnlyList<OutboxEvent>> ListForInvoiceAsync(
        Guid invoiceId,
        CancellationToken cancellationToken = default)
    {
        const string sql = $@"
    ...
    /// commits with the payment it announces or not at all.
    /// </summary>
    /// <remarks>
        ...
        CancellationToken cancellationToken)
    {
        const string sql = @"
            ...
            VALUES
                (@id, @projectId, @invoiceId, @eventType, @envelope, @occurredAt);";
