...
/// which Client owns the project (US-15).
/// </summary>
/// <remarks>
/// AC-1 requires a quotation to be viewable by the Client, and a role gate alone
/// would let any signed-in Client read any project's cost estimates by guessing
/// an id. The owning Client is a fact the Project Service holds, and this
/// service may not query that database — one schema per service. So the fact is
/// replicated locally off an event the Project Service already publishes; no new
/// event type is introduced on either side.
/// <para>
...
/// next poll retries it — and the ownership insert is idempotent, so a retry is
/// safe. Nothing here reaches back to the Project Service.
/// </para>
/// </remarks>
public sealed class ProjectEventsConsumer : BackgroundService
{
    ...
    /// </summary>
    /// <remarks>
    /// Suffixed rather than bare because this service reads more than one topic.
    ...
    /// makes each rebalance revoke the other's partitions, and the two consumers
    /// would take turns being assigned nothing. A group per topic keeps each
    /// one's offsets and assignment its own.
    /// </remarks>
    ...
    public async Task HandleAsync(string? value, CancellationToken cancellationToken)
    {
        ...
        {
            // ProjectUpdated, ProjectApproved and the construction lifecycle
            // moves — on the same topic, and none tells this service anything it
            // does not already have. A project is never reassigned to a different
            // Client, so ownership is settled at creation. Committed by the caller.
            return;
        }
