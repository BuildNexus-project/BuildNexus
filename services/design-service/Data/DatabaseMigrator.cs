...
/// <para>
/// Set up the same way as the User Service and the Project Service,
/// deliberately: one way of moving a schema forward across the whole system,
/// rather than a new one per service. Mounting the scripts into the MySQL
/// container's <c>docker-entrypoint-initdb.d</c> instead would only run them
/// when the data volume was brand new — so every schema change after the first
/// would need the volume, and all its data, thrown away.
/// </para>
/// <para>
...
public static class DatabaseMigrator
{
    ...
    /// </summary>
    /// <remarks>
    /// Does not create the database itself — <c>buildnexus_design_db</c> is
