...
/// <summary>
/// Publishes this service's events to Kafka on the <c>construction-events</c>
/// topic.
/// </summary>
/// <remarks>
/// One topic per publishing service, not one per event type: a consumer subscribes
/// to everything the Construction Service has to say and filters on the envelope's
/// <c>eventType</c> — the same way this service's own consumers read
/// <c>design-events</c> and <c>project-events</c>. A topic per event type would
/// multiply partitions for no gain and lose the ordering between two events about
/// the same project.
/// </remarks>
public sealed class KafkaConstructionEventPublisher : IConstructionEventPublisher, IDisposable
{
    /// <summary>The one topic this service publishes to.</summary>
    public const string Topic = "construction-events";

    private readonly IProducer<string, string> _producer;
    private readonly ILogger<KafkaConstructionEventPublisher> _logger;
    private readonly TimeSpan _flushTimeout;

    public KafkaConstructionEventPublisher(
        IOptions<KafkaOptions> options,
        ILogger<KafkaConstructionEventPublisher> logger)
    {
    ...
    public static ProducerConfig BuildProducerConfig(KafkaOptions kafkaOptions) => new()
    {
        ...
        // a lost ConstructionStarted means the Project Service never moves the
